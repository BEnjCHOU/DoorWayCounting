...
const double AREA2FILTERED = 10000.0;
...
int main()
{
    ...
    // Read video
    cv::VideoCapture video("../ManTrap_640p_20fps.mp4");
    // Web cam
    ...
    // image container
    cv::Mat frame, foreground, canny_filtered, final_output, bg;
   ...
   // while(1)
    {
        //video >> frame;
        //std::cout << frame.cols << " " << frame.rows << endl;
...
//        cv::imshow("frame", frame);
        ...
        BackgroundSubtraction->apply(frame, foreground); //,UPDATE_RATE);
        //BackgroundSubtraction->getBackgroundImage(bg);
        ...
        int erosion_type = 0;
        int erosion_size = 10; // do erosion 3 times
        ...
        // detect circle
        //        cv::Mat gray;
        //        cv::cvtColor(drawing_contours, gray, cv::COLOR_BGR2GRAY);
        ...
        //                     100, 30, 0, 300000 // change the last two parameters
        ...
        //            // circle center
        //            cv::circle( drawing_contours, center, 1, cv::Scalar(255,255,255), 3, cv::LINE_AA);
        //            // circle outline
        //            int radius = c[2];
        //            cv::circle( drawing_contours, center, radius, cv::Scalar(255,255,255), 3, cv::LINE_AA);
        //        }
        //        std::cout << "Circles detected size : " << circles.size() << std::endl;
